...
const WebSpeechPromptInput: React.FC<WebSpeechPromptInputProps> = ({ prompt, onSubmit, updatePrompt, speaking }) => {
  ...
  useEffect(() => {
    ...
    if (enableSpeech && !speaking && !recognizing) {
      console.log(`start speech recogition: ${recognition} recoginizing: ${recognizing} prompt: ${prompt}`);
  ...
  useEffect(() => {
    console.log(`useEffect: recognition ${recognition} recognizing ${recognizing} prompt:${prompt}`);
